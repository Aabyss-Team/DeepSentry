...
func buildRegistry() map[string]*Tool {
	all := []*Tool{
		...
		{Name: "network_device_baseline", Category: "网络设备", Description: "华为 VRP、H3C Comware、锐捷 RGOS、Cisco IOS 交换机/路由器只读基线：版本、板卡、接口、路由、STP、日志", RiskLevel: RiskLow, Perspective: PerspectiveTarget, ArgsHint: "profile(auto|huawei|h3c|ruijie|cisco)"},
		...
		{Name: "skill_market", Category: "配置管理", Description: "跨 ClawHub 与 skills.sh 搜索、检查、安装、更新、冻结和可恢复回滚 Agent Skill；全程记录来源锁", RiskLevel: RiskHigh, Perspective: PerspectiveController, ArgsHint: "action(search|inspect|install|managed|check_updates|update|pin|unpin|uninstall|rollback|audit), query, market, source, name, version, confirm_install/update/remove/rollback, acknowledge_risk, force, dest"},
