...
func buildRegistry() map[string]*Tool {
	all := []*Tool{
		...
		{Name: "netcat_probe", Category: "协议探测", Description: "TCP 端口探活 (Go net.Dial)", RiskLevel: RiskMedium, Perspective: PerspectiveController, ArgsHint: "host, port(必填)"},
		...
		{Name: "redis_probe", Category: "数据库探测", Description: "Redis RESP 只读探测 PING/INFO/CONFIG GET dir/dbfilename", RiskLevel: RiskMedium, Perspective: PerspectiveController, ArgsHint: "host, port(默认6379), password(可选), timeout"},
		{Name: "mysql_probe", Category: "数据库探测", Description: "MySQL/MariaDB handshake 解析版本、协议、capabilities", RiskLevel: RiskMedium, Perspective: PerspectiveController, ArgsHint: "host, port(默认3306), timeout"},
		{Name: "postgres_probe", Category: "数据库探测", Description: "PostgreSQL SSLRequest 识别 SSL 支持和协议响应", RiskLevel: RiskMedium, Perspective: PerspectiveController, ArgsHint: "host, port(默认5432), timeout"},
		{Name: "oracle_probe", Category: "数据库探测", Description: "Oracle TNS Listener 轻量连接探测", RiskLevel: RiskMedium, Perspective: PerspectiveController, ArgsHint: "host, port(默认1521), timeout"},
		...
		{Name: "awd_service_check", Category: "比赛辅助", Description: "AWD 服务可用性检查：控制端批量 HTTP/TCP 探活并汇总状态", RiskLevel: RiskLow, Perspective: PerspectiveController, ArgsHint: "targets(url/host:port逗号分隔), timeout"},
		...
		{Name: "archive_extract", Category: "文件传输", Description: "解压 zip/tar/tar.gz；远程 rar/7z 依赖目标系统 unrar/7z", RiskLevel: RiskHigh, Perspective: PerspectiveTarget, ArgsHint: "format, source, dest"},
