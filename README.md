...
```text
DeepSentry v2.0.1 Ultimate (build 2026-07-15)
```
...
| 60 个内置工具 | 覆盖网络、进程、日志、文件、文档、Web、数据库、pcap、Fleet、代理转发、定时任务、配置管理 |
...
MiMo 官方当前将 Coding/Agent 工具使用统一归在 Token Plan，MiMo Claw 也包含在套餐中；因此 DeepSentry 不会虚构一个未公开的独立 Agent Plan 地址。
...
| `browser_binary` | 指定 Chrome/Chromium 路径；留空时自动发现，不可用时回退 Go 静态网页解析 |
| `browser_timeout_sec` | 无头浏览器任务超时 |
...
| `/mcp list\|import\|add\|off\|on\|remove` | 管理 stdio MCP Server |
| `/skill list\|load\|unload\|add\|off\|on\|remove` | 管理当前会话 Skill 和 Skill 来源目录 |
| `/exit` / `/quit` | 退出 |

输入 `/` 会显示命令联想，输入 `/c` 可快速补全 `/clear`。

询问面板和最终报告都会解析 Markdown。表格在空间足够时显示为对齐网格，窗口较窄时自动改成逐条键值布局；Emoji、变体选择符、肤色与 ZWJ 组合按完整字素计算宽度。模型服务偶尔忽略协议、直接返回自然语言时，程序会自动区分用户询问、待执行的 Shell 代码块、未完成 JSON 和最终结论，不会把正常中文正文包装成 `invalid character ...` 解析错误。
...
当前版本注册 60 个内置工具。它们由 Go 原生实现或统一调度，Agent 会按需发现和调用，不会每轮把全部工具塞进 prompt。
...
同名配置会复用已有 stdio 子进程；外部 MCP 的副作用无法静态确认，因此交互模式默认要求用户确认，Batch 模式才会自动批准。
...
`build.sh` 会先执行项目自带的质量检查，再生成全平台二进制和 `build/SHA256SUMS`。构建完成后可校验产物：
...
如果只有 Emoji、Markdown 表格或询问框右边线错位，请先确认运行的是本次重新构建的二进制；新版使用字素簇宽度并在带边框面板绘制前做最终裁剪。确实不支持 Emoji 的旧终端仍可用 `DEEPSENTRY_PLAIN=1` 稳定降级。

如果模型偶尔不返回 JSON，新版会自动恢复普通 Markdown；残缺 JSON 会先要求模型重发，不再把解析器错误作为最终报告。若仍连续出现空响应，再检查模型兼容性、API 网关是否截断正文，以及 `llm_timeout_sec`。

如果看到 `Password:` 跑到 TUI 最底部，说明仍在运行旧二进制。先按 `Ctrl+C`（能回到 TUI 时也可按 `Esc`）中断卡住的命令，再启动最新 `build/deepsentry`；新版会自动进入系统 sudo 验证界面。也可在 TUI 空闲时执行 `/sudo`，或启动程序前在同一终端执行 `sudo -v`。不要使用 `echo 密码 | sudo -S`，也不要把 sudo 密码写进任务或配置。

系统 `Password:` 提示仍在屏幕上时，当前由 sudo 直接控制终端，此时滚轮属于系统终端而不是 TUI；完成输入、按 `Ctrl+C` 取消或连续三次验证失败后，新版会自动恢复全屏和鼠标模式。若返回 TUI 后滚轮仍露出 shell/sudo 历史，请重新构建并确认启动的是最新 `build/deepsentry`。
